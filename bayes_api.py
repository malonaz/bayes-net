...
class BayesNet:
    ...
    def combinations(self, variables, constant_bindings=None) :
        ...
        current_d = [merge_dicts(d,constant_bindings) for d in loop(unbound_variables)]
        # changed this implementations to remove duplicates
        return [dict(t) for t in set([tuple(d.items()) for d in current_d])]
